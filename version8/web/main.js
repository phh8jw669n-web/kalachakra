// main.js — Kalachakra v8 client. An 88-D relational SIREN globe over a world map.
//
// The whole 88-D network (33 local + 55 chords, 4x128 SIREN) runs PER VERTEX in shader8.js;
// the GPU interpolates colour across triangles. The CPU computes the 11 Earth-fixed body
// directions + GMST once per frame and hands them to the shader. Around the field: a world map
// (geo.js), the 11 bodies as 3D spheres orbiting the globe (planets8.js), and the full v5-style
// Temporal Helm + Observer HUD. Self-contained — no cross-version imports.

import * as THREE from "three";
import { OrbitControls } from "three/addons/controls/OrbitControls.js";
import { buildShaders, packWeights } from "./shader8.js";
import { equatorialDirs, gmstRad, BODY_NAMES, N_BODIES } from "./ephemeris8.js";
import { topocentricState } from "./state8.js";
import { makeSiren, boundLab, labToSrgb, srgbToHex } from "./siren8.js";
import {
...
import { createPlanets } from "./planets8.js";

const DEFAULT_ARCH = {
  in_features: 88, hidden: 128, hidden_layers: 4, out_features: 3, omega0: 30,
  lab_l0: 5, lab_lspan: 90, lab_ab: 80,
};
...
const app = {
  ...
  opacity: 0.85, tzMode: "local", tzOffsetMin: 0, seg: 128,
...
function randomWeights(a) {
  ...
  return { ...a, layers, output_activation: "v8_gamut", gamma: 15 };
}
...
function buildGlobe(weights) {
  const arch = {
    in_features: weights.in_features ?? 88, hidden: weights.hidden, hidden_layers: weights.hidden_layers,
    out_features: weights.out_features ?? 3, omega0: weights.omega0,
    lab_l0: weights.lab_l0 ?? 5, lab_lspan: weights.lab_lspan ?? 90, lab_ab: weights.lab_ab ?? 80,
  };
...
function updateReadouts(now) {
  ...
  if (!app.siren) return;
  const state = topocentricState(app.pin.lat, app.pin.lon, app.jd);   // 88-D
  for (let b = 0; b < N_BODIES; b++)
    matRows[b].textContent = ` ${fmt(state[b * 3])} ${fmt(state[b * 3 + 1])} ${fmt(state[b * 3 + 2])}`;
  const lab3 = boundLab(app.weights, app.siren(state));
...
main().catch((e) => { console.error("v8 main() failed:", e); setNotice("init error: " + e.message); clearBoot(); });
