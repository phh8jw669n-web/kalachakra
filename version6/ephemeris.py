...
def _ecl_dirs(jd):
    ...
    out = np.zeros((n, N_BODIES, 3), dtype=np.float64)
...
def topocentric_tensor(lat_deg, lon_deg, jd):
    """The 33-D local-sky tensor for a batch of observers/times.

    ``lat_deg, lon_deg, jd`` are broadcastable arrays ``[N]``. Returns ``[N, 33]``:
    for each of the 11 bodies its ``(North, East, Up)`` unit vector in the observer's
    horizontal frame.
    """
    ...
    dirs = _ecl_dirs(jd)                                      # [N,11,3] ecliptic
    eps = _obliquity(T)[:, None]                             # [N,1]
    xe = dirs[:, :, 0]
    ye = dirs[:, :, 1]
    ze = dirs[:, :, 2]
    ce, se = np.cos(eps), np.sin(eps)
    # ecliptic -> equatorial (rotate about X by +eps)
    xq = xe
    yq = ye * ce - ze * se
    zq = ye * se + ze * ce
    ra = np.arctan2(yq, xq)                                   # [N,11]
    dec = np.arcsin(np.clip(zq, -1.0, 1.0))

    lst = (gmst_deg(jd) * DEG)[:, None] + lon[:, None]        # local sidereal time [N,1]
    H = lst - ra                                             # hour angle [N,11]
    ...
    east = -cd * sH
    vec = np.stack([north, east, up], axis=-1)              # [N,11,3] unit vectors
